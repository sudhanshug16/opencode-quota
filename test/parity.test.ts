...
test("pinned registry inventory keeps unique providers and only fixture-backed support", () => {
  ...
  expect(supported.map(provider => provider.id)).toEqual(["codex", "clinepass", "opencodego", "fireworks", "copilot", "zai", "kimi", "moonshot", "openrouter", "deepseek", "deepinfra", "poe", "chutes", "v0"])
