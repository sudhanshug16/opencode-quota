...
The implemented collectors do not read cookies, browser stores, keychains or local provider databases. An API or auth error reports `unavailable` with no windows; unknown is never zero. Go percentages are in whole-percent units (`1` = 1%, `0.5` = 0.5%). These are percentages, **not** dollar amounts or request counts. Missing resets remain absent. No model-to-pool mapping is inferred. The [pinned upstream inventory](docs/parity.json) has 81 CodexBar providers; the [OpenCode scope partition](docs/opencode-scope.json) identifies **43 mapped provider families and 71 acquisition routes**, 32 excluded external products and six unresolved aliases. Seventeen routes across sixteen mapped providers have fixture-backed implementation; 54 acquisition routes remain, [triaged by capability/auth](docs/route-triage.json). Those routes are not 54 mandatory provider gaps: alternate auth and non-quota probes are tracked separately. The [credential-only completion matrix](docs/credential-matrix.json) separates eight source-aligned bridges, eight conditional bridges, and 27 mapped families unavailable **in this plugin**; no live account was verified. See [credential compatibility](docs/credential-compatibility.md). Native UI surfaces are inventoried in the upstream manifest but outside this TypeScript/OpenCode plugin surface.
...
Venice additionally requires `enableVenice: true`, `veniceConnectionId` and `veniceAccountLabel`. Hyper requires `enableHyper: true`, `hyperConnectionId` and `hyperAccountLabel`. Both are selected key connections; browser sessions are separate routes.
