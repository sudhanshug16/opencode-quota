...
No installation or config edit is performed by this repo. `enableGo`, `enableOpenRouter` and `enableDeepSeek` default to false. To enable OpenRouter or DeepSeek, set its `enable...` option to true and provide its non-empty `...AccountLabel`; the plugin uses only that provider's active existing **key** connection. The Go path uses only the active `opencode-go` key connection. OAuth and absent credentials yield unavailable. It does not export credentials or initiate authentication. The RPC returns `{ observations: Observation[] }`; its definition is exported from `opencode-quota/rpc` for [`@opencode/client` callers](https://opencode.ai/v2/docs/build/plugins/rpc). Plugin unload disposes the RPC and reader.
