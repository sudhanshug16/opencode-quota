...
| z.ai Coding Plan | Quota API and optional CN PAYG balance with mocked personal/team fixtures; model history unported | Opt-in existing `zai-coding-plan` key connection with explicit account label, connection ID, region and `zaiScope: "personal"`; CN balance additionally needs `enableZaiCnBalance: true`; portable callback also accepts explicit team organization/project | Credit/token/MCP lanes, plausible resets and separate CNY PAYG balance if authorized; no inferred model route mapping |
...
| Chutes | Direct subscription endpoint with pinned-source fixture; quota-list enrichment unported | Opt-in selected `chutes` key connection and account label | Rolling/monthly request or credit allowances and reset when returned; inactive plan distinct from zero quota |
...
The implemented collectors do not read cookies, browser stores, keychains or local provider databases. An API or auth error reports `unavailable` with no windows; unknown is never zero. Go percentages are in whole-percent units (`1` = 1%, `0.5` = 0.5%). These are percentages, **not** dollar amounts or request counts. Missing resets remain absent. No model-to-pool mapping is inferred. The [pinned upstream inventory](docs/parity.json) has 81 CodexBar providers; the [OpenCode scope partition](docs/opencode-scope.json) identifies **43 mapped provider families and 71 acquisition routes**, 32 excluded external products and six unresolved aliases. Twenty routes across eighteen mapped providers have fixture-backed implementation; 51 acquisition routes remain, [triaged by capability/auth](docs/route-triage.json). Those routes are not 51 mandatory provider gaps: alternate auth and non-quota probes are tracked separately. The [credential-only completion matrix](docs/credential-matrix.json) separates eight source-aligned bridges, ten conditional bridges, and 25 mapped families unavailable **in this plugin**; no live account was verified. See [credential compatibility](docs/credential-compatibility.md). Native UI surfaces are inventoried in the upstream manifest but outside this TypeScript/OpenCode plugin surface.
